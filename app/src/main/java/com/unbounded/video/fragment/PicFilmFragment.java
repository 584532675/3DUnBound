...
public class PicFilmFragment extends BaseFragment {
    ...
    private void initPics(){
        ...
        if(!(TextUtils.isEmpty(selectStart)) && !("null".equals(selectStart)) && currentPage != 1){
            url = HttpConstants.selectInfoByTypeImg_Url + "?type=movie" + "&currentPage="+currentPage + "&selectStart="+selectStart;
        }else {
            url = HttpConstants.selectInfoByTypeImg_Url + "?type=movie" + "&currentPage=" + currentPage;
        }
                ...
                if (resultCode == -1) {
                    ...
                    InteractionManager.getInstance(getActivity(), new ActionCallback() {
                        ...
                        public void onRequset(int returnCode) {
                             ...
                             * 激活结果
                             */
                            phoneNum = SharedPrefsUtil.getValue(getActivity(), Constants.Phone_FLAG, "");
                            ZhugeSDK.getInstance().track(getActivity(),"激活结果总次数"+phoneNum);
...
//                                    ToastUtil.showToast(getActivity(), "激活成功,请开始眼球追踪校准",ToastUtil.BOTTOM);
                                     ...
                                     */
                                    phoneNum = SharedPrefsUtil.getValue(getActivity(), Constants.Phone_FLAG, "");
                                    ZhugeSDK.getInstance().track(getActivity(),"激活成功"+phoneNum);
