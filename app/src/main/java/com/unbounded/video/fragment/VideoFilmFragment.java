...
public class VideoFilmFragment extends BaseFragment {
    ...
    private void initFilms(){
        ...
        if(!(TextUtils.isEmpty(selectStart)) && !("null".equals(selectStart)) && currentPage != 1){
            url = HttpConstants.SelectInfoByType_VideosUrl + "?type=movie" + "&currentPage=" + currentPage + "&selectStart="+selectStart;
        }else {
            url = HttpConstants.SelectInfoByType_VideosUrl + "?type=movie" + "&currentPage=" + currentPage;
        }
    ...
    public void initView(View view) {
        ...
        adapter = new VideoOnlineAdapter(VideoFilmFragment.this, getActivity(), onlineVideos, onlineImgWidth, oneDp, screenHeight);

        footer = LayoutInflater.from(getActivity()).inflate(R.layout.load_more_footer,null ,false);
        swipeRefreshLayout = (SwipeRefreshLayout) view.findViewById(R.id.videorecomend_swiperefresh);
        rela1 = (RelativeLayout) view.findViewById(R.id.videosrecommend_rela1);
        lv = (ListView) view.findViewById(R.id.videorecomend_lv);
                    ...
                    if(isLoading == false){
                        if(currentPage < sumPage){
                            ...
                            if(currentPage > 0){
                                isLoading = false;
                                footer.setVisibility(View.GONE);
                                ToastUtil.showToast(getActivity(), R.string.not_have_more, ToastUtil.BOTTOM);
                            }
                        }
                    }
                }
